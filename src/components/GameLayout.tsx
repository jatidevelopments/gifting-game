      ...
      <nav className="mb-6">
        <ul className="flex justify-center items-center gap-1">
          {gameSteps.map((step, index) => {
            ...
            const isPast = gameSteps.findIndex(s => s.href === currentPath) > index;

            return (
              <li key={step.href} className="flex items-center">
                <Link
                  href={`/game/${gameId}/${step.href}`}
                  className={`px-3 py-2 rounded-lg transition-colors flex flex-col items-center text-center border font-cinzel
                    ${isActive 
                      ? 'bg-purple-500/20 text-white border-purple-500/30' 
                      : isPast
                        ? 'bg-purple-500/10 text-white/80 border-purple-500/20 hover:bg-purple-500/15'
