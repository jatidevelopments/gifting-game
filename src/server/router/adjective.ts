...
The adjectives should inspire gift-givers and help them think creatively about their gift choices.`;
        ...
        try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a helpful assistant that generates descriptive adjectives for gift recommendations. Always return adjectives one per line, all lowercase, no numbers or special characters.",
              },
              {
                role: "user",
                content: prompt,
          ...
          if (adjectives.length === 0) {
            console.error(
              "No valid adjectives generated from response:",
              response,
            );
            throw new AppError(
              "INTERNAL_SERVER_ERROR",
              "Failed to generate valid adjectives",
            );
          }

          if (adjectives.length < input.count) {
            console.warn(
              `Generated only ${adjectives.length} valid adjectives out of ${input.count} requested`,
            );
          }
  ...
  generateForAllCategories: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const prompt = `As a Secret Santa gift exchange assistant, generate ${input.count} unique and descriptive adjectives in ${input.language} language for the category "${category.name}". These adjectives will be used to inspire and guide gift selections. The response should ONLY contain the adjectives in ${input.language}, one per line.

Category-specific guidelines for "${category.name}" (but respond in ${input.language}):
${
  category.name === "color"
    ? `
- Focus on specific, vivid color descriptions
...
Requirements:
1. Generate exactly ${input.count} adjectives in ${input.language}
2. Each adjective must be:
   - A single word in ${input.language}
   - Appropriate for gift descriptions
   - Easy to understand for all ages
   - Positive or neutral in connotation
3. Avoid:
...
Format:
- Return only the adjectives in ${input.language}
- One adjective per line
- No numbers, bullets, or additional formatting
- All lowercase
            ...
            messages: [
              {
                role: "system",
                content:
                  `You are a helpful assistant that generates descriptive adjectives for gift recommendations in ${input.language}. Always return adjectives one per line, all lowercase, no numbers or special characters.`,
              },
              {
                role: "user",
                content: prompt,
          ...
          const adjectives = response
            ...
            .filter((adj) => {
              ...
              const isValid =
                adj.length > 0 &&
                adj.length <= 20 && // reasonable length
                /^[a-zÀ-ÿ]+$/u.test(adj) && // letters including accents
                !adj.includes(" "); // single word
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (!adjective) {
          throw new AppError(
            "NOT_FOUND",
            "Adjective not found in this game room",
          );
        }
